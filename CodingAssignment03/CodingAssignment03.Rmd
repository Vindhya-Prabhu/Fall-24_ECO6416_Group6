...
```{r q1}
#
```randomRowNumbers <- sample(seq_len(nrow(insurance)), size = 30)
 Insurance$lnCustomer <- log(insurance$customer) 
Insurance$age <- log(Insurance$age)
Insurance$ageSquared <- Insurance$age^2

train <- Insurance[-randomRowNumbers,]
test <- Insurance[randomRowNumbers,]
train %>% 
...
```{r}
#
```cor(train %>% select(lnCustomer,lnage,ageSquared,age,BMI)) %>% 
