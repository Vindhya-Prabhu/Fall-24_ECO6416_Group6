...
```{r q1}
#Univariate Analysis: Mean of the variable Age is greater than its median thus it is right skewed. Hence, mean is no longer a good measure of center for the univariate analysis using the variable Age, instead median should be used. Mean of the variables BMI & Children are approximately equal to their median thus they are symmetric, however, since the variable is BMI multi-modal and Children doesn't contain any mode, they are not normal distribution and cannot use mean and standard deviation. 
...
var(InsuranceData$BMI)
var(InsuranceData$Children)

sd(InsuranceData$Age)
sd(InsuranceData$BMI)
sd(InsuranceData$Children)

IQR(InsuranceData$Age)
IQR(InsuranceData$BMI)
IQR(InsuranceData$Children)

range(InsuranceData$Age)
range(InsuranceData$BMI)
range(InsuranceData$Children)

boxplot(InsuranceData$Age)
boxplot(InsuranceData$BMI)
boxplot(InsuranceData$Children)
...
model <- lm(Charges ~ InsuranceData$Age + InsuranceData$BMI + InsuranceData$Children, data = InsuranceData)
